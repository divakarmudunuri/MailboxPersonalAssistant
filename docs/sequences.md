# Sequence diagrams

How the main flows move through the app. Names match the modules in [architecture.md](architecture.md).

## 1. A new email arrives

![seq-new-email](images/seq-new-email.svg)

<sub>Source: [diagrams/seq-new-email.mmd](diagrams/seq-new-email.mmd)</sub>

## 2. The inbox manager acts

![seq-inbox-manager](images/seq-inbox-manager.svg)

<sub>Source: [diagrams/seq-inbox-manager.mmd](diagrams/seq-inbox-manager.mmd)</sub>

## 3. Saving a category by hand in the Triage tab

![seq-manual-save](images/seq-manual-save.svg)

<sub>Source: [diagrams/seq-manual-save.mmd](diagrams/seq-manual-save.mmd)</sub>

## 4. Teaching a preference in the Memory tab

![seq-memory-chat](images/seq-memory-chat.svg)

<sub>Source: [diagrams/seq-memory-chat.mmd](diagrams/seq-memory-chat.mmd)</sub>

## 5. Building the inbox briefing

![seq-briefing](images/seq-briefing.svg)

<sub>Source: [diagrams/seq-briefing.mmd](diagrams/seq-briefing.mmd)</sub>

## 6. Startup and authentication

![seq-startup](images/seq-startup.svg)

<sub>Source: [diagrams/seq-startup.mmd](diagrams/seq-startup.mmd)</sub>
